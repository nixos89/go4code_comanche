	  ...
	  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if (user == null) {
		  throw new UsernameNotFoundException(String.format("No user found with username '%s'.", username));
			...
			//Java 1.8 way   	
			List<GrantedAuthority> grantedAuthorities = user.getUserSecurityAuthorities().stream()
		            .map(authority -> new SimpleGrantedAuthority(authority.getSecurityAuthority().getName()))
